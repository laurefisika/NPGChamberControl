...
- Technical documentation and traceable evolution from early prototypes to release `0.9.21`.

## Workflow

| Phase | Purpose | Main integrations |
| --- | --- | --- |
| 01 — Heat up + Calibration | Heat and degas the molecular evaporator, then calibrate the QMB thickness ratio with selectable temperature, rate or compound feedback | QMBs, XGS600, oven PID, Keysight supply, CK-1 Arduino, pyrometer |
...
| 03 — DP-DBBA Evaporation | Deposit the molecular precursor with selectable temperature, rate or compound feedback | QMBs, XGS600, oven PID, Keysight supply, CK-1 Arduino, pyrometer |
...
The latest supported snapshot is `0.9.21`. It adds continuation runs for Phase 02, safe 0 °C completion for Phases 02 and 04, selectable CK-1 temperature/rate/compound feedback for Phases 01 and 03, and corrected package imports for phase subprocesses launched from their data folders. The complete release history from `0.8.5` onward is recorded in [CHANGELOG.md](CHANGELOG.md). The [historical archive](history/README.md) includes the March prototypes, sixteen recovered July project packages, and the standalone COSCON and pyrometer diagnostics.
...
Release `0.9.21` records 125 passing no-hardware regression tests on 30 July 2026. The suite covers packaging, launcher behavior, parameter validation, serial handoff and import resolution, pyrometer integration, COSCON dashboard logic, safe-zero completion, CK-1 rate/compound feedback, pressure alarms, and source-manifest integrity. The new rate loop is software-tested but still requires the supervised chamber checks in [RATE_PID_VALIDATION.md](RATE_PID_VALIDATION.md).

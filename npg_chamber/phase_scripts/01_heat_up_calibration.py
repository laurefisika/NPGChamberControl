...
final_folder_path = str(create_numbered_run_dir("heat", safe_sample_name, parent=script_folder))
...
# The Sample QMB must remain at/above the calibration target for this long
# before the endpoint is accepted. The timer advances on fresh Sample-QMB
# thickness timestamps, so a stale reading cannot satisfy the hold by itself.
CALIBRATION_TARGET_STABLE_S = 5.0
# Calibration quality still audits synchronized QMB linearity. The final ratio
# itself is frozen from the live relative QMB values at stable-target confirmation,
# not from the first instantaneous target crossing.
...
# Separate from the cascade tracker: this one only decides when the Sample-QMB
# calibration endpoint has been continuously above target for long enough.
calibration_target_stable_tracker = StableConditionTracker(CALIBRATION_TARGET_STABLE_S)
...
def confirm_shutter_open(source: str = 'GUI button'):
    ...
    calibration_target_stable_tracker.reset()
    ...
    message = (
        ...
        f"{ck1_baseline:.2f} Å, Sample: {sample_baseline:.2f} Å."
        if ck1_baseline is not None and sample_baseline is not None
...
def confirm_shutter_closed(source: str = 'GUI button'):
    ...
    ck1_absolute = latest_ck1_thickness()
    sample_absolute = latest_sample_thickness()
...
def latest_sample_thickness_timestamp():
    times = data['Sample QMB']['thickness_times']
    return times[-1] if times else None
...
def _add_shutter_region_to_snapshot(axes):
    """Mark Open/Close Shutter and shade the accepted calibration interval."""
        ...
        if close_time is not None:
            ...
            if close_time >= open_time:
                region = ax.axvspan(
                    ...
                    color='#0f766e',
                    alpha=0.075,
                    label='Calibration region' if index == 0 else '_nolegend_',
                )
                if index == 0:
...
def calculate_calibration_result(sample_rel=None, ck1_rel=None, confirmation_timestamp=None):
    """Freeze the calibration ratio at stable-target confirmation.

    The first target crossing is retained only as an audit/linearity diagnostic.
    The ratio handed to Phase 03 is CK-1 relative thickness / Sample relative
    thickness from the live values present after the Sample QMB has remained at
    or above target for ``CALIBRATION_TARGET_STABLE_S`` continuously.
    """
    crossing_diagnostic = exact_calibration_ratio(
        sample_times=data['Sample QMB']['thickness_times'],
    ...
    try:
        endpoint_ratio = float(ck1_rel) / float(sample_rel) if float(sample_rel) != 0.0 else None
    ...
    if isinstance(confirmation_timestamp, datetime):
        confirmation_iso = confirmation_timestamp.isoformat()
    else:
        try:
            confirmation_iso = datetime.fromtimestamp(float(confirmation_timestamp)).isoformat()
        except Exception:
            confirmation_iso = datetime.now().isoformat()

    quality_pass = bool(crossing_diagnostic.get('quality_pass')) and endpoint_ratio is not None
    if endpoint_ratio is None:
        quality_message = 'REVIEW / REPEAT RECOMMENDED: stable-confirmation ratio could not be calculated.'
        quality_reason = 'stable-confirmation ratio could not be calculated'
    elif quality_pass:
        quality_message = (
            f'PASS: Sample QMB remained at/above target for {CALIBRATION_TARGET_STABLE_S:.1f} s '
            'and synchronized QMB linearity passed.'
        )
        quality_reason = 'accepted'
    else:
        quality_reason = crossing_diagnostic.get('reason') or 'calibration quality check failed'
        quality_message = f'REVIEW / REPEAT RECOMMENDED: {quality_reason}'

    return {
        'ratio': endpoint_ratio,
        'confirmation_timestamp': confirmation_iso,
        'sample_target_a': float(CALIBRATION_TARGET_SAMPLE_A),
        'target_stable_duration_s': float(CALIBRATION_TARGET_STABLE_S),
        'sample_relative_at_confirmation_a': sample_rel,
        'ck1_relative_at_confirmation_a': ck1_rel,
        'first_crossing_timestamp': crossing_diagnostic.get('crossing_timestamp'),
        'first_crossing_ratio': crossing_diagnostic.get('ratio'),
        'ck1_relative_at_first_crossing_a': crossing_diagnostic.get('ck1_relative_at_crossing_a'),
        'fit_slope_ratio': crossing_diagnostic.get('fit_slope_ratio'),
        'linearity_r2': crossing_diagnostic.get('linearity_r2'),
        'quality_pass': quality_pass,
        'quality_message': quality_message,
        'synchronized_fit_points': crossing_diagnostic.get('synchronized_fit_points'),
        'reason': quality_reason,
    }


def calculate_thickness_ratio():
    """Return the frozen stable-confirmation ratio, or a live ratio before completion."""
...
def save_phase_summary(tag: str):
    ...
    try:
        with open(path, 'w', encoding='utf-8') as f:
            ...
            for field in (
                'confirmation_timestamp', 'sample_target_a', 'target_stable_duration_s',
                'sample_relative_at_confirmation_a', 'ck1_relative_at_confirmation_a',
                'first_crossing_timestamp', 'first_crossing_ratio',
                'ck1_relative_at_first_crossing_a', 'fit_slope_ratio', 'linearity_r2',
                'quality_pass', 'quality_message', 'synchronized_fit_points',
            ):
...
def process_controller():
    ...
    while not stop_event.is_set():
        ...
        if phase == 'HEATING_UP':
            ...
            sample_rel = relative_sample_thickness()
            sample_measurement_timestamp = latest_sample_thickness_timestamp()
            try:
                sample_measurement_s = sample_measurement_timestamp.timestamp()
            except Exception:
                sample_measurement_s = None

            target_condition = (
                sample_rel is not None
                and sample_measurement_s is not None
                and float(sample_rel) >= float(CALIBRATION_TARGET_SAMPLE_A)
            )
            if sample_measurement_s is None:
                calibration_target_stable_tracker.reset()
                target_confirmed = False
            else:
                target_confirmed = calibration_target_stable_tracker.update(
                    target_condition,
                    sample_measurement_s,
                )

            if target_confirmed:
                # Freeze the ratio here, before the operator physically closes the shutter.
                # This avoids adding the manual close/button reaction delay to the calibration.
                ck1_rel = relative_ck1_thickness()
                ...
                if thickness_ratio is not None:
                    ratio_message = (
                        f'Stable-endpoint thickness ratio (CK-1 / Sample after '
                        f'{float(CALIBRATION_TARGET_STABLE_S):.1f} s continuously >= '
                ...
                set_phase(
                    'WAIT_SHUTTER_CLOSE',
                    f'Sample relative thickness stayed >= {CALIBRATION_TARGET_SAMPLE_A:.2f} Å for '
                    f'{CALIBRATION_TARGET_STABLE_S:.1f} s; waiting for shutter close'
                )
                print_banner(
                    f"TARGET CONFIRMED: Sample relative thickness = {sample_rel:.2f} Å\n"
                    f"Target remained >= {CALIBRATION_TARGET_SAMPLE_A:.2f} Å for "
                    f"{CALIBRATION_TARGET_STABLE_S:.1f} s.\n"
                    f"{ratio_message}\n"
...
def qt_dashboard_status():
    ...
    rel_ck1 = relative_ck1_thickness()
    rel_sample = relative_sample_thickness()
    abs_ck1 = latest_ck1_thickness()
    abs_sample = latest_sample_thickness()

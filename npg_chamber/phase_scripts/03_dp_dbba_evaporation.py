...
from npg_chamber.common.paths import create_numbered_run_dir, safe_run_name
...
OVEN_READY_TOLERANCE_C = RUN_AUTOMATION_OVERRIDES.get("OVEN_READY_TOLERANCE_C", 2.0)
OVEN_READY_STABLE_DURATION_S = RUN_AUTOMATION_OVERRIDES.get("OVEN_READY_STABLE_DURATION_S", 60.0)
OVEN_READY_SIGNAL_TIMEOUT_S = RUN_AUTOMATION_OVERRIDES.get("OVEN_READY_SIGNAL_TIMEOUT_S", 10.0)
...
safe_sample_name = safe_run_name(sample_name, fallback="unnamed_trial")
final_folder_path = str(create_numbered_run_dir("dpdbba", safe_sample_name, parent=script_folder))
...
# Small electrical-zero threshold used only for output-state diagnostics.
# Phase 03 has no controlled ramp-down path: Abort is immediate 0 A / OUTPUT OFF,
# while normal Finish hands off at KEYSIGHT_BASE_WORK_CURRENT_A for Phase 04.
ZERO_CURRENT_THRESHOLD_A = 0.003
...
oven_ready_tracker = StableBandTracker(OVEN_READY_TOLERANCE_C, OVEN_READY_STABLE_DURATION_S)
...
process_state = {
    ...
    'shutter_open_timestamp': None,
    'shutter_close_timestamp': None,
...
def build_run_info_text():
    return (
        ...
        f"watchdog_maximum_temperature_c: {TEMP_WATCHDOG_MAX_TEMP_C:.3f}\n"
    )
...
# tiny off-screen canvas only for diagnostic helper calls; report/snapshot figures are
...
def confirm_shutter_open(source: str = 'GUI button'):
    shutter_open_timestamp = time.time()
    ck1_baseline = latest_ck1_thickness()
    sample_baseline = latest_sample_thickness()
    with state_lock:
        # Capture a real pre-reset baseline immediately at the operator action.
        # The process thread will then reset the Phase-03 thickness window to 0 Å;
        # these values keep the GUI relative trace correct during that handover.
        process_state['baseline_ck1_thickness'] = ck1_baseline
        process_state['baseline_sample_thickness'] = sample_baseline
        process_state['shutter_open_timestamp'] = shutter_open_timestamp
        process_state['shutter_close_timestamp'] = None
        process_state['shutter_open_confirmed'] = True
        process_state['shutter_close_confirmed'] = False

    ck1_absolute = cumulative_qmb_thickness_for_display(
        'CK-1 evaporator QMB', ck1_baseline
    )
    sample_absolute = cumulative_qmb_thickness_for_display(
        'Sample QMB', sample_baseline
    )
    message = (
        f"Shutter OPEN confirmed from {source}. "
        f"Absolute thickness at opening — CK-1: "
        f"{ck1_absolute:.2f} Å, Sample: {sample_absolute:.2f} Å."
        if ck1_absolute is not None and sample_absolute is not None
        else f"Shutter OPEN confirmed from {source}; absolute thickness is not yet available."
    )
    print_banner(message)
    _set_live_action_status(message)
...
def confirm_shutter_closed(source: str = 'GUI button'):
    shutter_close_timestamp = time.time()
    with state_lock:
        process_state['shutter_close_timestamp'] = shutter_close_timestamp
        process_state['shutter_close_confirmed'] = True
        process_state['shutter_open_confirmed'] = False

    ck1_relative = relative_ck1_thickness()
    sample_relative = relative_sample_thickness()
    ck1_absolute = cumulative_qmb_thickness_for_display(
        'CK-1 evaporator QMB', latest_ck1_thickness()
    )
    sample_absolute = cumulative_qmb_thickness_for_display(
        'Sample QMB', latest_sample_thickness()
    )
    message = (
        f"Shutter CLOSED confirmed from {source}. "
        f"CK-1 absolute/relative: "
        f"{ck1_absolute:.2f}/{ck1_relative:.2f} Å; "
        f"Sample absolute/relative: "
        f"{sample_absolute:.2f}/{sample_relative:.2f} Å."
        if None not in (ck1_absolute, sample_absolute, ck1_relative, sample_relative)
        else f"Shutter CLOSED confirmed from {source}."
    )
    print_banner(message)
    _set_live_action_status(message)
...
def request_gui_abort(event=None):
    """Abort Phase 03 immediately and close only this phase process.

    Emergency priority is the evaporator power: command the Keysight to 0 A /
    OUTPUT OFF immediately.  After that, make a best-effort request for Oven
    PID SV=0 °C and save shutdown diagnostics.  This path never performs the
    slow controlled abort ramp-down; Phase 03 no longer has such a path. Normal
    completion instead hands off at 0.640 A / OUTPUT ON for Phase 04.
    """
    message = (
        'Abort requested from GUI. Keysight is being commanded immediately to '
        '0 A / OUTPUT OFF; Oven PID 0.0 °C will be requested best-effort and '
        'Phase 03 will then close.'
    )
    print_banner(message)
    _set_live_action_status(message)
    ...
    # Hardware-safe priority: remove evaporator power immediately.  Setting
    # stop_event also makes the individual Qt dashboard/process exit while the
    # unified launcher remains alive.
    emergency_keysight_shutdown('GUI Abort button - immediate phase stop')

    # The oven is a separate device. Request its safe target only after the
    # Keysight has already been switched OFF, so a slow PID transaction can
    # never delay evaporator shutdown.
    try:
        set_oven_pid_setpoint(0.0)
        _set_live_action_status(
            'Abort complete: Keysight 0 A / OUTPUT OFF; Oven PID target 0.0 °C requested.'
        )
    except Exception as e:
        warning = (
            f'Abort warning: Keysight is already OFF, but Oven PID 0.0 °C could not be '
            f'confirmed: {e}. Verify the PID manually.'
        )
        print_banner(warning)
        _set_live_action_status(warning)
...
def request_gui_finish(event=None):
    """Request the normal Phase 03 handoff without changing its sequence.

    Normal DP-DBBA completion is valid only after the target has been reached
    and the operator has physically closed the shutter.  This button therefore
    remains blocked until WAIT_SHUTTER_CLOSE and then records the same shutter-
    closed confirmation used by the established controller.  The controller
    still performs the normal handoff and leaves the Keysight ON at base current
    for Phase 04; this is intentionally different from Abort / safe stop.
    """

    phase = current_phase()
    if phase != 'WAIT_SHUTTER_CLOSE':
        message = (
            'Finish phase is not available yet. Wait until the DP-DBBA target is reached, '
            'physically close the shutter, then click Finish phase.'
        )
        print_banner(message)
        _set_live_action_status(message)
        return

    confirm_shutter_closed('GUI Finish phase button')
    message = (
        'Normal Phase 03 finish confirmed. The controller will complete the handoff '
        'and leave the Keysight ON at base current for Phase 04.'
    )
    print_banner(message)
    _set_live_action_status(message)
...
def setup_live_target_controls():
    ...
    btn_abort = add_button(0.06, 0.166, 0.40, 0.036, 'Abort / safe stop', '#fee2e2', '#fecaca', '#991b1b', fontsize=7.5)
    btn_abort.on_clicked(request_gui_abort)
    live_target_buttons['abort'] = btn_abort

    btn_finish = add_button(0.52, 0.166, 0.40, 0.036, 'Finish phase', '#e0f2fe', '#bae6fd', '#075985', fontsize=7.5)
...
def update_live_dashboard_text(snapshot=None):
    ...
    lines = [
        ...
        f'Oven ready: {oven_ready_tracker.elapsed(time.monotonic()):.0f}/{OVEN_READY_STABLE_DURATION_S:.0f} s',
...
def oven_ready_for_evaporation():
    oven_pv = latest_value('Oven PID temperature', 'temperature_data')
    times = data['Oven PID temperature']['temperature_times']
    age_s = None
    if times:
        try:
            age_s = max(0.0, time.time() - times[-1].timestamp())
        except Exception:
            age_s = None
    if age_s is None or age_s > OVEN_READY_SIGNAL_TIMEOUT_S:
        oven_ready_tracker.reset()
        return False
    ready = oven_ready_tracker.update(oven_pv, OVEN_TARGET_TEMPERATURE_C, time.monotonic())
    return bool(ready)


def heating_ready_for_shutter(ck1_temp, ck1_rate_avg):
    """Return True when the Phase 03 physical prerequisites are met.

    The external oven keeps its real 200 °C stability prerequisite.  Once that
    prerequisite is satisfied, CK-1 shutter progression uses only the
    operator-facing CK-1 targets: temperature at/above target and averaged QMB
    rate at/above target.  Controller diagnostics do not gate the transition.
    """
...
def _add_shutter_region_to_snapshot(axes):
    """Mark Open/Close Shutter and shade the accepted evaporation interval."""
    open_timestamp = process_state.get('shutter_open_timestamp')
    close_timestamp = process_state.get('shutter_close_timestamp')
    try:
        open_time = datetime.fromtimestamp(float(open_timestamp))
    except Exception:
        return
    try:
        close_time = datetime.fromtimestamp(float(close_timestamp))
    except Exception:
        close_time = None
    ...
    for index, ax in enumerate(axes_list):
        open_line = ax.axvline(
            open_time,
            linestyle='--',
            linewidth=1.35,
            color='#0f766e',
            alpha=0.95,
            label='Open shutter' if index == 0 else '_nolegend_',
        )
        if index == 0:
            legend_handles.append(open_line)
        if close_time is not None:
            close_line = ax.axvline(
                close_time,
                linestyle='--',
                linewidth=1.35,
                color='#c2410c',
                alpha=0.95,
                label='Close shutter' if index == 0 else '_nolegend_',
            )
            if index == 0:
                legend_handles.append(close_line)
            if close_time >= open_time:
                region = ax.axvspan(
                    ...
                    color='#0f766e',
                    alpha=0.075,
                    label='Evaporation region' if index == 0 else '_nolegend_',
                )
                if index == 0:
                    legend_handles.append(region)
    if legend_handles:
        axes_list[0].legend(handles=legend_handles, loc='best', fontsize=7.5)
...
def save_graphs_only_snapshot(tag: str, path: str, snapshot=None):
    ...
    snapshot_fig.subplots_adjust(left=0.065, right=0.985, top=0.92, bottom=0.075, hspace=0.42, wspace=0.38)
    ...
    if any(
        ...
        for value in source['XGS600 HFIG pressure']['pressure_data']
    ):
        fax_pressure_xgs600.set_yscale('log')
    _plot_series_for_snapshot(
    ...
    _add_shutter_region_to_snapshot(axes.flat)
...
def save_phase_summary(tag: str):
    ...
    try:
        with open(path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"shutter_open_timestamp: {process_state.get('shutter_open_timestamp')}\n")
            f.write(f"shutter_close_timestamp: {process_state.get('shutter_close_timestamp')}\n")
    ...
    while not stop_event.is_set():
        try:
            ...
            unexpected_output_off = (
                ...
                and current_phase() not in ('FINISHED', 'SAFETY_STOP')
            )
...
def process_controller():
    print_banner(
        ...
        "Use the GUI buttons for Open Shutter, Close Shutter, Abort / safe stop, and Finish phase. "
        ...
        if phase == 'HEATING_UP':
            ...
            if heating_ready_for_shutter(ck1_temp, ck1_rate_avg):
                trigger_reason = (
                    ...
                    f'External oven stability at {OVEN_TARGET_TEMPERATURE_C:.1f} ºC is verified. '
                    f'Feedback mode: {evaporation_control_mode_label()}.'
                )
...
def qt_dashboard_status():
    ...
    rel_ck1 = relative_ck1_thickness()
    rel_sample = relative_sample_thickness()
    abs_ck1 = cumulative_qmb_thickness_for_display(
        'CK-1 evaporator QMB', latest_ck1_thickness()
    )
    abs_sample = cumulative_qmb_thickness_for_display(
        'Sample QMB', latest_sample_thickness()
    )
    ...
    lines = [
        ...
        f'DP-DBBA CK-1 target: {TARGET_CK1_THICKNESS_A:.2f} Å',
        _qt_value_line('CK-1 absolute', abs_ck1, '.2f', 'Å'),
        _qt_value_line('Sample absolute', abs_sample, '.2f', 'Å'),
        ...
        _qt_value_line('Oven PV', latest_value('Oven PID temperature', 'temperature_data'), '.1f', 'ºC'),
        f'Oven required: {OVEN_TARGET_TEMPERATURE_C:.1f} ± {OVEN_READY_TOLERANCE_C:.1f} ºC for {OVEN_READY_STABLE_DURATION_S:.0f} s',
        f'Oven PV timeout: {OVEN_READY_SIGNAL_TIMEOUT_S:.0f} s',
    ...
    return {
        ...
        'ramp': ramp,
        'manual': manual,
        'shutter_open_timestamp': process_state.get('shutter_open_timestamp'),
        'shutter_close_timestamp': process_state.get('shutter_close_timestamp'),
        'relative_thickness_active': process_state.get('shutter_open_timestamp') is not None,
        'baseline_ck1_thickness': process_state.get('baseline_ck1_thickness'),
        'baseline_sample_thickness': process_state.get('baseline_sample_thickness'),
    }
...
def build_qt_dashboard_spec():
    ...
    return PhaseDashboardSpec(
        window_title='Phase 03 · DP-DBBA Evaporation',
        phase_name='DP-DBBA Evaporation',
...
def main():
    ...
    try:
        if USE_QT_PHASE13_DASHBOARD:
            pass
        elif AUTO_CLOSE_WHEN_LAUNCHED_FROM_UNIFIED:
            plt.close('all')
        else:

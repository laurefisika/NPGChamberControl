...
def _ck1_common(*, include_calibration_target: bool, include_rampdown: bool = True) -> tuple[ParameterSpec, ...]:
    specs: list[ParameterSpec] = [
        ...
        ParameterSpec("PID_MAX_STEP_A", "Legacy PID maximum correction", 0.0025, unit="A", group="Temperature PID", description="Compatibility value for older recipes; professional control uses the time-normalized slew limits below.", minimum=0.00001, maximum=0.05),
...
PHASE_PARAMETER_SPECS: dict[str, tuple[ParameterSpec, ...]] = {
    ...
    "sputter": (
        ...
        ParameterSpec("coscon_activation_overload_retries", "Activation overload retries", 1, kind="int", group="COSCON safety margins", description="Automatic recovery attempts allowed only for HV-Module Energy Overload during COSCON activation. A repeated overload still aborts safely.", minimum=0, maximum=2),
        ...
        ParameterSpec("stable_temperature_reads", "Legacy stable readings", 3, kind="int", group="Temperature detection", description="Compatibility value; continuous stable time is the primary criterion.", minimum=1, maximum=1000),

...
def request_gui_abort(event=None):
    message = (
        'Abort requested from GUI. The script will set the Oven PID target to 0.0 ºC, '
        'run the controlled abort ramp-down and then switch the Keysight output OFF.'
    )
    print_banner(message)
    _set_live_action_status(message)
    ...
    def _abort_rampdown_worker():
        try:
            ...
            _set_live_action_status(
                'Abort: Oven PID target confirmed at 0.0 ºC. Ramping the Keysight output down.'
            )
        except Exception as e:
            warning = (
                f'Abort warning: could not confirm the Oven PID 0.0 ºC target: {e}. '
                'Continuing the Keysight ramp-down and output-OFF sequence.'
            )

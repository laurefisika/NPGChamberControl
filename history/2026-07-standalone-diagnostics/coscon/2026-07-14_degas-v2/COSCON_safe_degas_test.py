...
Default test sequence:
    Off -> Degassing -> observe for 10 s -> Off
...
above the configured start limit. During Degas it requests Off immediately if pressure reaches the documented
1e-4 mbar cutoff, then verifies the final state.
...
def request_safe_stop(
    ...
    """Best-effort documented safe stop during Degassing: request Off directly."""
...
def show_preflight(args: argparse.Namespace) -> None:
    print(
        ...
        f"{args.observe_seconds:.1f} seconds, then request Off.\n"
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if mode in {"off", "standby"}:
                ...
                break
            if mode not in {"degas", "degassing"}:
                raise TestError(

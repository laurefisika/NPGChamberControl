...
class CosconUDP:
    ...
    def send(self, command: str) -> str:
        ...
        # Reject only a command-level error. A valid status reply such as
        # 'GetStatus OK: Mode=Error ...' must remain parseable.
        command_name = command.split()[0]
        command_error = re.compile(
            rf"^{re.escape(command_name)}\s+ERROR\b", re.IGNORECASE
        )

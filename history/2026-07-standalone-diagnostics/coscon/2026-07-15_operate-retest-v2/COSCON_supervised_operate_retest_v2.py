#!/usr/bin/env python3
"""
Supervised COSCON Operating re-test v2.
...
DEFAULT_HOLD_S = 3.0
DEFAULT_TRANSITION_TIMEOUT_S = 45.0
DEFAULT_POLL_S = 0.75

CONFIRMATION = "OPERATE RETEST 10mA 2250V"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Supervised COSCON Operating re-test v2."
    )
...
def preflight_message(args: argparse.Namespace) -> None:
    print(
        ...
        "2. A complete Degas cycle has been performed; initial Mode may be Off or Standby.\n"
...
def capture_snapshot(client: CosconUDP, logger: ReportLogger, label: str) -> None:
    """Record a diagnostic snapshot without changing COSCON state."""
    logger.add(f"SNAPSHOT START [{label}]")
    for command in (
        "GetStatus",
        "GetTargetValues",
        "GetMonitorValues",
        ...
        try:
            ...
            logger.add(f"SNAPSHOT [{label}] {command} failed: {exc}")
    logger.add(f"SNAPSHOT END [{label}]")


def wait_for_operating_with_diagnostics(
    client: CosconUDP,
    gauge: XGS600,
    logger: ReportLogger,
    args: argparse.Namespace,
):
    """Poll state, pressure and diagnostics during the HV ramp."""
    deadline = time.monotonic() + args.transition_timeout
    ...
    while time.monotonic() < deadline:
        ...
        last_status = status
        require_interlock_ok(status, "during the Operating transition")
        ...
        if mode == "operating":
            return status
        if mode == "error":
            capture_snapshot(client, logger, "MODE ERROR DURING RAMP")
            raise TestError(
                f"COSCON entered Mode=Error during ramp: {status.details}"
            )
        if mode not in {"off", "standby", "switchingtooperate"}:
            raise TestError(
                f"Unexpected mode during Operating transition: "
                f"{status.mode} ({status.details})"
            )

        now = time.monotonic()
        if now >= next_snapshot:
            capture_snapshot(client, logger, "OPERATING RAMP")
            next_snapshot = now + 2.0

        time.sleep(args.poll)

    raise TestError(
        "Timeout waiting for Mode=Operating. Last status: "
        + (last_status.raw if last_status else "no status received")
    )
...
def main() -> int:
    ...
    try:
        ...
        if status.mode.lower() not in {"off", "standby"}:
            raise TestError(
                f"Test requires initial Mode=Off or Standby, but COSCON "
                f"reports {status.mode}."
            )
        ...
        operating = wait_for_operating_with_diagnostics(
            client, gauge, logger, args
        )
        logger.add(
            f"Operating confirmed: Interlock={operating.interlock}, "
            f"Details={operating.details!r}"
        )
        capture_snapshot(client, logger, "OPERATING CONFIRMED")
        ...
        while time.monotonic() < deadline:
            ...
            if status.mode.lower() == "error":
                capture_snapshot(client, logger, "MODE ERROR DURING HOLD")
                raise TestError(
            ...
            capture_snapshot(client, logger, "OPERATING HOLD")
        ...
        report = logger.save(
            ...
            filename_prefix="coscon_operate_retest_v2",
            title="COSCON IS SUPERVISED OPERATING RE-TEST V2",
            result=result,
            reason=reason,

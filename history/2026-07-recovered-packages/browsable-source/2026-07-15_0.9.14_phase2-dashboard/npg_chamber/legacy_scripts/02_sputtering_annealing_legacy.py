  ...
  <style>
    ...
    * { box-sizing:border-box; }
    html, body {
      ...
      color:var(--text);
      background:
        radial-gradient(circle at 8% 0%, rgba(96,165,250,.20), transparent 30%),
        radial-gradient(circle at 92% 0%, rgba(167,139,250,.12), transparent 25%),
        linear-gradient(180deg, var(--bg), var(--bg2));
    }

    body.theme-degassing { --blue:#a78bfa; --blue2:#7c3aed; }
    body.theme-sputter   { --blue:#f59e0b; --blue2:#b45309; }
    body.theme-anneal    { --blue:#fb7185; --blue2:#be123c; }
    body.theme-done      { --blue:#22c55e; --blue2:#15803d; }
    body.theme-abort     { --blue:#ef4444; --blue2:#991b1b; }
    ...
    .card {
      background:linear-gradient(180deg, rgba(21,38,64,.96), rgba(15,29,49,.98));
    ...
    .header {
      padding:16px 18px;
      display:grid;
      grid-template-columns:minmax(320px,1fr) minmax(580px,1.35fr);
      align-items:center;
      gap:18px;
    }
    ...
    .title { font-size:25px; font-weight:950; letter-spacing:.1px; }
    ...
    .progressWrap { grid-column:1 / -1; margin-top:10px; }
    .progressLine {
      height:8px;
      border-radius:999px;
      overflow:hidden;
    ...
    .main {
      min-height:0;
      display:grid;
      grid-template-columns:minmax(430px,1.12fr) minmax(390px,.92fr) minmax(330px,.78fr);
      gap:12px;
    }
    ...
    .sectionTitle { font-size:14px; font-weight:950; letter-spacing:.2px; }
    .pill {
      padding:5px 9px;
      border-radius:999px;
    ...
    .stageHero {
      ...
      border-radius:17px;
      background:
        radial-gradient(circle at 95% 0%, rgba(255,255,255,.09), transparent 28%),
        linear-gradient(135deg, rgba(96,165,250,.17), rgba(37,99,235,.05));
    ...
    .stageText { margin-top:10px; font-size:19px; font-weight:950; }
    .stageDetails {
      margin-top:7px;
      color:var(--muted);
      font-size:12px;
    ...
    button {
      ...
      font-weight:900;
      cursor:pointer;
      background:linear-gradient(180deg, #334a68, #23344c);
    ...
    button.success { background:linear-gradient(180deg, #22c55e, #15803d); }
    button.warning { background:linear-gradient(180deg, #f59e0b, #b45309); }
    button.abort {
      width:100%;
      margin-top:10px;
    ...
    .timerCard strong { font-size:21px; font-weight:950; }
    .timerNote { margin-top:8px; color:var(--muted); font-size:10px; line-height:1.35; }

    .workflow { display:flex; flex-direction:column; gap:6px; }
    .step {
      display:grid;
    ...
    .step.done .stepNumber { background:#15803d; color:#fff; }
    .step.active {
    ...
    .stepName { font-size:11px; font-weight:900; }
    .stepDesc { color:var(--muted); font-size:9px; margin-top:2px; }
    .stepTime { color:#c8d8ee; font-size:9px; font-weight:850; text-align:right; }
    ...
    .metricSub { margin-top:4px; color:var(--muted); font-size:9px; line-height:1.3; }
    ...
    .pidRow { display:grid; grid-template-columns:1fr auto; gap:7px; margin-top:8px; }
    input {
      min-width:0;
      padding:10px;
      border-radius:12px;
    ...
    .pidStatus { margin-top:7px; color:#bfdbfe; font-size:10px; line-height:1.35; }
    ...
    @media (max-width:1180px) {
      ...
      .column:last-child { grid-column:1 / -1; display:grid; grid-template-columns:1fr 1fr; }
    }
  </style>
</head>
<body>
  <div class="shell">
    <header class="card header">
      <div>
        ...
        <div class="subtitle">Direct COSCON UDP control. The COSCON web page is not used. The argon leak valve remains manual.</div>
      </div>
      <div>
        <div class="topStats">
          ...
          <div class="topStat"><span>Interlock</span><strong id="headerInterlock">--</strong></div>
        </div>
        <div class="progressWrap">
          <div class="progressLine"><div id="progressFill" class="progressFill"></div></div>
          <div class="progressMeta">
            <span id="progressText">Workflow progress: 0%</span>
            <span>First integrated runs must remain supervised</span>
          </div>
        </div>
      </div>
    </header>
    ...
    <main class="main">
      <section class="column">
        ...
        <div class="card section">
          <div class="sectionHeader">
            ...
            <div class="pill">Live countdowns</div>
          </div>
          <div class="timerGrid">
            <div class="timerCard"><span id="currentTimerLabel">Current step</span><strong id="currentTimer">--:--</strong></div>
            <div class="timerCard"><span>Current-stage elapsed</span><strong id="stageElapsed">00:00</strong></div>
          ...
          <div class="timerNote">Known timed work excludes manual valve actions and the oven heating ramp, whose duration depends on the chamber response.</div>
        </div>
      ...
      <section class="column">
        ...
        <div class="card section">
          <div class="sectionHeader">
            <div class="sectionTitle">Oven and auxiliary readback</div>
            <div class="pill">Live</div>
          </div>
          <div class="metrics">
            <div class="metric">
              <div class="metricLabel">Oven PV</div>
              <div id="pvVal" class="metricValue">-- °C</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Oven SV</div>
              <div id="svVal" class="metricValue">-- °C</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Keysight voltage</div>
              <div id="keysightV" class="metricValue">-- V</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Keysight current</div>
              <div id="keysightI" class="metricValue">-- A</div>
            </div>
          </div>
        </div>

        <div id="pidControlCard" class="card section" style="display:none">
          <div class="sectionHeader">
            <div class="sectionTitle">Live PID SV control</div>
            <div class="pill">S1 write + verify</div>
          </div>
          <div class="pidBox">
            <div>Use only when the operator intentionally needs to change the oven setpoint during the run.</div>
            <div class="pidRow">
              <input id="pidSvInput" type="number" step="1" inputmode="decimal" placeholder="New SV, e.g. 620">
              <button id="setSvBtn" class="primary" type="button">Set PID SV</button>
            </div>
      ...
      <section class="column">
        <div class="card section">
          <div class="sectionHeader">
            ...
            <div id="updatedAt" class="pill">No telemetry</div>
          </div>
        ...
        <div class="card section">
          <div class="sectionHeader">
            <div class="sectionTitle">Operator reminders</div>
          </div>
          <div class="footer">
            Keep the COSCON web interface and SpecsLab/Prodigy closed while this phase is active.<br><br>
            The script automates Degas, target validation, Operate, output verification, sputtering timing and Standby. Only the leak-valve opening and closing remain manual.<br><br>
            Keep access to the local COSCON controls and supervise the first complete three-cycle runs.
          </div>
        </div>
      </section>
    </main>
...
<script>
  ...
  const WORKFLOW_STEPS = [
    {key:'DEGASSING', name:'COSCON Degas', desc:'Automatic in cycle 1', time:'automatic'},
  ...
  function renderWorkflow(snap) {
    ...
    WORKFLOW_STEPS.forEach((step, idx) => {
      ...
      if (step.key === 'DEGASSING' && snap.degas_expected_s) {
        stepTime = `expected ${formatDuration(snap.degas_expected_s)}`;
      }
...
class SputterAnnealController:
    ...
    def _estimated_timed_remaining_s(self) -> int:
        ...
        timed = {
            "DEGASSING": int(self.cfg.expected_degassing_minutes * 60),
            ...
            snap.update(
                {
                    ...
                    "degas_expected_s": int(self.cfg.expected_degassing_minutes * 60),
    ...
    def coscon_degassing_step(self, cycle: int) -> None:
        ...
        self._set_phase_timer(total_s, total_s, "Degas max left")
        while time.time() < deadline:

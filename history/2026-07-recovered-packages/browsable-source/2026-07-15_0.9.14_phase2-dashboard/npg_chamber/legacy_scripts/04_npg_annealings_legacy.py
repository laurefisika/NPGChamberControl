...
def update_figure(fig: plt.Figure, artists: dict, state: SharedState) -> None:
    ...
    _set_series(artists["ax_oven"], artists["line_oven"], oven_times, oven_values)
...
def save_pid_temperature_curve(path: str, state: SharedState, run_name: str) -> str:
    ...
    ax.set_facecolor("white")
    ax.set_title(
        f"NPG Annealings · PID temperature curve\nRun: {run_name}",
        fontsize=14,
        fontweight="bold",
        color="#0f172a",
        pad=14,
    ...
    ax.set_ylabel("Oven PID temperature (°C)", fontsize=10.5)
    ...
    if times and temperatures:
        ...
        ax.plot(times, temperatures, linewidth=1.9, marker=marker, markersize=markersize)
...
class App:
    def __init__(self, run_name: str) -> None:
        ...
        self.pid_temperature_curve_path = os.path.join(self.output_dir, "01__PID_temperature_curve.png")
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            info(f"Saved PID temperature curve: {self.pid_temperature_curve_path}", Fore.CYAN)
        except Exception as exc:

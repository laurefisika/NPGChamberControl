...
## 0.9.12 - COSCON IS read-only UDP diagnostic (same version)

- Added a strict read-only COSCON IS UDP client for the officially documented ASCII protocol on port 2005.
- Added `diagnostic_tools/check_coscon_read_only.py` to read device identity, firmware, serial number, network configuration, operating status, monitor values, diagnostic values, target values and preset slots.
- Added `RUN_COSCON_READ_ONLY_CHECK.bat` for a simple Windows launch from the project folder.
- The diagnostic enforces a command whitelist and rejects all state-changing commands, including Operate, Degas, Standby, Off, Reset, network changes and preset write/delete operations.
- Diagnostic results are saved as timestamped JSON and text reports in `COSCON Diagnostic Reports/`.
- Added regression tests for CR termination, UDP port 2005, response parsing, timeout retry and rejection of write/control commands.
- Kept the package version at `0.9.12`; Phase 02 still uses the existing operator-controlled COSCON workflow and no experimental control behavior was changed.

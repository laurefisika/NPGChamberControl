...
class NPGLauncherApp:
    def __init__(self) -> None:
        ...
        self.pyrometer_parameter_values = pyrometer_default_values()
    ...
    def open_automation_parameters(self) -> None:
        ...
        tk.Label(
            ...
            text=(
                ...
                "sets the material profile used by Phases 01 and 03. COM ports and baud rates remain fixed; "
        ...
        # Shared pyrometer profile. It is configured once in the launcher and
        # passed to Phases 01 and 03 through a dedicated environment variable.
        pyro_bg = "#f3efff"
        pyro_accent = "#8b70d8"
        ...
        tk.Label(
            intro,
            text=(
                "This profile is used by both Phase 01 and Phase 03. The pyrometer remains monitoring-only: "
                "it never changes PID, Keysight, shutter, phase-transition or safety decisions. All three "
                "temperature series are logged even when the graph shows only one."
            ),
            bg="#ffffff",
            fg="#475569",
            font=("Segoe UI", 9),
        ...
        def load_validated_au_profile() -> None:
            ...
            try:
                ...
                for spec in pyrometer_specs():
                    ...
                    if spec.kind == "bool":
                        var.set(bool(defaults[spec.key]))
                    elif spec.key == "default_view":
                        var.set(pyrometer_view_labels[str(defaults[spec.key])])
                    else:
                        var.set(spec.format_display(defaults[spec.key]))
            finally:
                profile_update_guard["active"] = False
        ...
        def mark_pyrometer_profile_custom(*_args) -> None:
            if profile_update_guard["active"]:
                return
            profile_var = pyro_vars["profile_name"]
            if profile_var.get() == "Au/mica — validated":
                profile_var.set("Custom material")
        ...
        tk.Button(
            pyro_content,
            text="Load validated Au/mica profile",
            bg="#c4b5fd",
            fg="#111827",
            activebackground="#a78bfa",
            relief="solid",
            ...
            pady=7,
            font=("Segoe UI", 9, "bold"),
            command=load_validated_au_profile,
        ).grid(row=row, column=0, columnspan=2, sticky="w", padx=14, pady=(12, 16))
        ...
        def set_phase_to_defaults(phase_key: str) -> None:
            specs = pyrometer_specs() if phase_key == "pyrometer" else specs_for_phase(phase_key)
            for spec in specs:
                var = editor_vars[phase_key][spec.key]
                if spec.kind == "bool":
                    var.set(bool(spec.default))
                elif phase_key == "pyrometer" and spec.key == "default_view":
                    var.set(pyrometer_view_labels[str(spec.default)])
                else:
        ...
        def apply_changes() -> None:
            ...
            validated_pyrometer: dict[str, object] = {}
            try:
                ...
                raw_pyrometer: dict[str, object] = {}
                for spec in pyrometer_specs():
                    var = editor_vars["pyrometer"][spec.key]
                    raw_value = var.get()
                    if spec.key == "default_view":
                        raw_value = pyrometer_view_values[str(raw_value)]
                    raw_pyrometer[spec.key] = spec.internal_value(raw_value)
                validated_pyrometer = validate_pyrometer_values(raw_pyrometer)
            ...
            if total_changed:
                ...
                self.status_var.set("Automation and pyrometer parameters reset to the packaged defaults.")
            dialog.destroy()

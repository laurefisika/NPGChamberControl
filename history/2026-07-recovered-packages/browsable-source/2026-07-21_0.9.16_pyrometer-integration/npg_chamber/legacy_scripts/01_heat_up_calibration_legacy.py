...
fig, ((ax_thickness_ck1, ax_rate_ck1, ax_pressure_xgs600),
      ...
      (ax_current_keysight, ax_voltage_keysight, ax_temperature_ck1)) = plt.subplots(3, 3, figsize=(19, 11.5))
fig.patch.set_facecolor('#f4f6f8')
plt.subplots_adjust(left=0.06, right=0.76, top=0.92, bottom=0.08, hspace=0.34, wspace=0.28)
plt.ion()

line_thickness_ck1, = ax_thickness_ck1.plot([], [], linewidth=1.8, color="green")
line_rate_ck1, = ax_rate_ck1.plot([], [], linewidth=1.8, color="green")
line_thickness_sample, = ax_thickness_sample.plot([], [], linewidth=1.8, color="green")
line_rate_sample, = ax_rate_sample.plot([], [], linewidth=1.8, color="green")
line_pressure_xgs600, = ax_pressure_xgs600.plot([], [], linewidth=1.8, color="blue")
line_temperature_oven, = ax_temperature_oven.plot([], [], linewidth=1.8, color="magenta")
line_current_keysight, = ax_current_keysight.plot([], [], linewidth=1.8, color="goldenrod")
line_voltage_keysight, = ax_voltage_keysight.plot([], [], linewidth=1.8, color="goldenrod")
line_temperature_ck1, = ax_temperature_ck1.plot([], [], linewidth=1.8, color="red")

AXIS_INFO_STYLE = dict(boxstyle='round,pad=0.25', facecolor='white', edgecolor='0.82', alpha=0.9)

plot_axes_config = [
    (ax_thickness_ck1, 'CK-1 thickness', 'Thickness (Å)'),
    (ax_rate_ck1, 'CK-1 rate', 'Rate (Å/s)'),
    (ax_thickness_sample, 'Sample thickness', 'Thickness (Å)'),
    (ax_rate_sample, 'Sample rate', 'Rate (Å/s)'),
    (ax_pressure_xgs600, 'Pressure', 'Pressure (mbar)'),
    (ax_temperature_oven, 'Oven temp', 'Temperature (ºC)'),
    (ax_current_keysight, 'Current', 'Current (A)'),
    (ax_voltage_keysight, 'Voltage', 'Voltage (V)'),
    (ax_temperature_ck1, 'CK-1 temp', 'Temperature (ºC)'),
]

for ax, title, ylabel in plot_axes_config:
...
def _temperature_view_label(mode):
    return {
        ...
        'pyrometer': f'Raw pyrometer temperature · ε {PYROMETER_PROFILE.emissivity_percent:.1f}%',
        'sample': f'Estimated sample temperature · {PYROMETER_PROFILE.profile_name}',
...
def set_temperature_view(mode):
    ...
    ax_temperature_oven.set_title(_temperature_view_label(mode), fontsize=11, pad=28)
...
def setup_live_target_controls():
    ...
    panel_text(0.05, 0.957, 'Live controls and operator actions', fontsize=8.1, color='#475569')

    # Editable heating targets
    panel_text(0.05, 0.925, 'Editable targets', fontsize=9.3, color='#334155', weight='bold')
    target_box_specs = [
        ('trigger_temp_c', 'Temp target (ºC)', f"{HEATING_TRIGGER_TEMP_C:.1f}", 0.872),
        ('rate_target_a_per_s', 'Target CK-1 rate (Å/s)', f"{CK1_RATE_TARGET_A_PER_S:.3f}", 0.817),
        ('pid_temp_band_c', 'PID band (ºC)', f"{PID_TEMP_BAND_C:.2f}", 0.762),
    ]
    for key, label, initial, y_rel in target_box_specs:
        add_textbox(key, label, initial, y_rel, target='targets')

    btn_apply = add_button(0.08, 0.715, 0.37, 0.035, 'Apply', '#dbeafe', '#bfdbfe', '#1e3a8a', fontsize=8.8)
    btn_apply.on_clicked(_apply_live_targets_from_widgets)
    live_target_buttons['apply'] = btn_apply

    btn_reset = add_button(0.52, 0.715, 0.37, 0.035, 'Reset', '#e2e8f0', '#cbd5e1', '#334155', fontsize=8.8)
    btn_reset.on_clicked(_reset_live_targets_to_defaults)
    live_target_buttons['reset'] = btn_reset

    # Ramp-up mode controls
    panel_text(0.05, 0.702, 'Ramp-up mode', fontsize=9.3, color='#334155', weight='bold')
    btn_steps = add_button(0.06, 0.642, 0.40, 0.034, 'Steps ramp', '#eef2ff', '#c7d2fe', '#312e81', fontsize=7.1)
    btn_steps.on_clicked(_set_steps_ramp_mode)
    live_ramp_buttons[RAMP_MODE_STEPS] = btn_steps

    btn_slope = add_button(0.52, 0.642, 0.40, 0.034, 'Slope ramp', '#eff6ff', '#bae6fd', '#075985', fontsize=7.1)
    btn_slope.on_clicked(_set_slope_ramp_mode)
    live_ramp_buttons[RAMP_MODE_SLOPE] = btn_slope

    add_textbox('steps_until_temp_c', 'Steps until T (ºC)', f"{STEPS_RAMP_UNTIL_TEMP_C:.1f}", 0.604, target='ramp')
    add_textbox('steps_step_period_s', 'Step period (s)', f"{STEPS_RAMP_STEP_PERIOD_S:.1f}", 0.564, target='ramp')
    add_textbox('slope_early_c_per_min', 'Slope early (ºC/min)', f"{TEMP_SLOPE_TARGET_EARLY_C_PER_MIN:.2f}", 0.524, target='ramp')
    add_textbox('slope_mid_c_per_min', 'Slope mid (ºC/min)', f"{TEMP_SLOPE_TARGET_MID_C_PER_MIN:.2f}", 0.484, target='ramp')
    add_textbox('slope_late_c_per_min', 'Slope late (ºC/min)', f"{TEMP_SLOPE_TARGET_LATE_C_PER_MIN:.2f}", 0.444, target='ramp')

    btn_ramp_apply = add_button(0.08, 0.402, 0.37, 0.032, 'Apply ramp', '#ede9fe', '#ddd6fe', '#5b21b6', fontsize=7.2)
    btn_ramp_apply.on_clicked(_apply_live_ramp_settings_from_widgets)
    live_ramp_buttons['apply_ramp'] = btn_ramp_apply

    btn_ramp_reset = add_button(0.52, 0.402, 0.37, 0.032, 'Reset ramp', '#f1f5f9', '#e2e8f0', '#334155', fontsize=7.2)
    ...
    panel_text(0.05, 0.382, 'Manual Keysight current', fontsize=9.1, color='#334155', weight='bold')
    add_textbox('manual_current_a', 'Manual I (A)', f"{KEYSIGHT_START_CURRENT_A:.3f}", 0.337, target='manual')

    btn_manual_set = add_button(0.06, 0.294, 0.40, 0.034, 'Set manual I', '#fee2e2', '#fecaca', '#991b1b', fontsize=6.8)
    btn_manual_set.on_clicked(_apply_manual_current_from_widgets)
    live_manual_buttons['set_manual_current'] = btn_manual_set

    btn_auto_current = add_button(0.52, 0.294, 0.40, 0.034, 'Back to Auto', '#dcfce7', '#bbf7d0', '#166534', fontsize=6.8)
    btn_auto_current.on_clicked(_resume_automatic_current_control)
    live_manual_buttons['resume_auto_current'] = btn_auto_current

    # Operator actions
    panel_text(0.05, 0.275, 'Operator actions', fontsize=9.3, color='#334155', weight='bold')
    btn_open = add_button(0.06, 0.224, 0.40, 0.036, 'Open Shutter', '#dcfce7', '#bbf7d0', '#166534', fontsize=6.6)
    btn_open.on_clicked(_gui_open_shutter)
    live_target_buttons['open_shutter'] = btn_open

    btn_close = add_button(0.52, 0.224, 0.40, 0.036, 'Close Shutter', '#ffedd5', '#fed7aa', '#9a3412', fontsize=6.6)
    btn_close.on_clicked(_gui_close_shutter)
    live_target_buttons['close_shutter'] = btn_close

    btn_abort = add_button(0.06, 0.180, 0.40, 0.036, 'Abort', '#fee2e2', '#fecaca', '#991b1b', fontsize=7.8)
    btn_abort.on_clicked(request_gui_abort)
    live_target_buttons['abort'] = btn_abort

    btn_finish = add_button(0.52, 0.180, 0.40, 0.036, 'Finish', '#e0f2fe', '#bae6fd', '#075985', fontsize=7.8)
    btn_finish.on_clicked(request_gui_finish)
    live_target_buttons['finish'] = btn_finish

    # Run status and last action
    panel_text(0.05, 0.154, 'Run status', fontsize=9.3, color='#334155', weight='bold')
    live_dashboard_text = control_panel_ax.text(
        0.05, 0.139, '', transform=control_panel_ax.transAxes,
        fontsize=6.15, color='#0f172a', va='top', ha='left', linespacing=0.88,
        clip_on=True
    )

    panel_text(0.55, 0.154, 'Last action', fontsize=9.3, color='#334155', weight='bold')
    live_target_status_text = control_panel_ax.text(
        0.55, 0.139, '', transform=control_panel_ax.transAxes,
        fontsize=6.15, color='#334155', va='top', ha='left', linespacing=0.88,
        clip_on=True
    )
...
def update_live_plot(snapshot, force_autoscale=False):
    ...
    if temperature_mode == 'pyrometer':
        ...
        elif pyro_last is not None and pyro_last < PYROMETER_PROFILE.minimum_valid_pyrometer_c:
            display_info = f'below calibrated range\nTpyro {pyro_last:.1f} ºC'
        else:
    ...
    ax_temperature_oven.set_title(_temperature_view_label(temperature_mode), fontsize=11, pad=28)
    _update_axis(
    ...
    fig.suptitle(phase_title_for_display(phase), fontsize=18, fontweight='bold', y=0.975)
    try:
        fig.canvas.draw_idle()
    except Exception:
        pass
...
def read_pyrometer():
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if PYROMETER_PROFILE.write_emissivity_at_start:
                if not emissivity_setup_attempted:
                    # Only one automatic write attempt is allowed per phase. If it
                    # fails, the chamber phase continues and later retries remain
                    # read-only rather than repeatedly writing the instrument.
                    ...
                    if abs(confirmed - PYROMETER_PROFILE.emissivity_percent) > 0.11:
                        raise RuntimeError(
                            'Selected pyrometer emissivity is not active; the single automatic '
                            'write attempt has already been used for this phase'
                        )
            else:
                confirmed = reader.read_emissivity_percent()
            ...
            while not stop_event.is_set():
                ...
                try:
                    ...
                    status = (
                        'OK'
                        if math.isfinite(sample_c)
                        else f'below calibrated range (<{PYROMETER_PROFILE.minimum_valid_pyrometer_c:.1f} C)'
                    )
                    _append_pyrometer_reading(timestamp, raw_c, sample_c, status)
                    sample_text = f'{sample_c:.1f} ºC' if math.isfinite(sample_c) else 'below calibrated range'
                    print(
                        f"{formatted}.{Fore.LIGHTBLACK_EX}{dec}{Style.RESET_ALL} - "
                        f"{Fore.CYAN}Pyrometer: {raw_c:.1f} ºC | Estimated sample: {sample_text}{Style.RESET_ALL}"
                    )
                except Exception as exc:

...
def setup_live_target_controls():
    ...
    target_box_specs = [
        ...
        ('pid_temp_band_c', 'PID band (ºC)', f"{PID_TEMP_BAND_C:.1f}", 0.762),
    ]
    ...
    # Operator actions
    panel_text(0.05, 0.288, 'Operator actions', fontsize=9.3, color='#334155', weight='bold')
    btn_open = add_button(0.06, 0.229, 0.40, 0.036, 'Open Shutter', '#dcfce7', '#bbf7d0', '#166534', fontsize=6.6)
    btn_open.on_clicked(_gui_open_shutter)
    live_target_buttons['open_shutter'] = btn_open

    btn_close = add_button(0.52, 0.229, 0.40, 0.036, 'Close Shutter', '#ffedd5', '#fed7aa', '#9a3412', fontsize=6.6)
    btn_close.on_clicked(_gui_close_shutter)
    live_target_buttons['close_shutter'] = btn_close

    btn_abort = add_button(0.06, 0.183, 0.86, 0.038, 'Abort ramp-down', '#fee2e2', '#fecaca', '#991b1b', fontsize=7.2)
    btn_abort.on_clicked(request_gui_abort_or_finish)
    live_target_buttons['abort_finish'] = btn_abort

    # Run status and last action
    panel_text(0.05, 0.174, 'Run status', fontsize=9.8, color='#334155', weight='bold')
    live_dashboard_text = control_panel_ax.text(
        0.05, 0.157, '', transform=control_panel_ax.transAxes,
        fontsize=6.2, color='#0f172a', va='top', ha='left', linespacing=0.88,
        clip_on=True
    )

    panel_text(0.55, 0.174, 'Last action', fontsize=9.8, color='#334155', weight='bold')
    live_target_status_text = control_panel_ax.text(
        0.55, 0.157, '', transform=control_panel_ax.transAxes,
        fontsize=6.2, color='#334155', va='top', ha='left', linespacing=0.88,
        clip_on=True
    )

...
<head>
  ...
  <style>
    ...
    :root {
      ...
      --cyan:#2f9fb3;
      --pink:#d66d91;
      --shadow:0 8px 22px rgba(35,54,75,.08);
    }
    ...
    * { box-sizing:border-box; }
    ...
    body.theme-degassing { --blue:#8b70d8; --blue2:#6e53bd; }
    body.theme-sputter   { --blue:#e99a32; --blue2:#ca751d; }
    body.theme-anneal    { --blue:#d66d91; --blue2:#b94972; }
    body.theme-done      { --blue:#2ca66f; --blue2:#1e8457; }
    body.theme-abort     { --blue:#dc5b65; --blue2:#b73f49; }
    ...
    .accent-violet { border-top:4px solid #8b70d8; background:#f4effd; }
    .accent-blue   { border-top:4px solid #5b8def; background:#edf4ff; }
    .accent-cyan   { border-top:4px solid #2f9fb3; background:#ecf9fb; }
    .accent-amber  { border-top:4px solid #e99a32; background:#fff4e8; }
    .accent-pink   { border-top:4px solid #d66d91; background:#fff0f6; }
    .accent-green  { border-top:4px solid #2ca66f; background:#edf9f2; }
    .accent-slate  { border-top:4px solid #7a8ca2; background:#f1f4f8; }

    .header {
      padding:13px 18px;
      display:grid;
      grid-template-columns:minmax(290px,.92fr) minmax(620px,1.45fr);
      align-items:center;
      gap:16px;
    }
    .eyebrow {
    ...
    .title { font-size:28px; font-weight:950; letter-spacing:.1px; color:#203247; }
    .subtitle { margin-top:4px; color:var(--muted); font-size:12px; line-height:1.35; }

    .topStats { display:grid; grid-template-columns:repeat(4,minmax(118px,1fr)); gap:8px; }
    .topStat {
      padding:9px 11px;
    ...
    .topStat small { display:block; margin-top:3px; color:#8a9aae; font-size:8.5px; }

    .progressWrap { grid-column:1 / -1; margin-top:7px; }
    .progressLine { height:7px; border-radius:999px; overflow:hidden; background:#e8eef6; }
    .progressFill {
      width:0%; height:100%;
      background:linear-gradient(90deg, var(--blue), var(--blue2));
      transition:width .3s ease;
    }
    .progressMeta { display:flex; justify-content:space-between; gap:12px; color:var(--muted); font-size:9.8px; margin-top:4px; }

    .main {
      min-height:0;
      overflow:hidden;
      display:grid;
      grid-template-columns:minmax(430px,1.12fr) minmax(360px,.92fr) minmax(410px,1.02fr);
      gap:10px;
    }
    .column { min-height:0; overflow:hidden; display:flex; flex-direction:column; gap:10px; }
    .section { padding:11px 13px; overflow:hidden; }
    .sectionHeader { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:9px; }
    .sectionTitle {
      font-size:14px;
      font-weight:950;
      letter-spacing:.9px;
    ...
    .accent-violet .sectionTitle { color:#6f55bd; }
    .accent-blue .sectionTitle { color:#376fd0; }
    .accent-cyan .sectionTitle { color:#247f91; }
    .accent-amber .sectionTitle { color:#b96c19; }
    .accent-pink .sectionTitle { color:#ad4168; }
    .accent-green .sectionTitle { color:#1f8256; }
    .pill {
      padding:3px 7px;
      border-radius:999px;
    ...
    .workflowCard { flex:1 1 auto; min-height:0; display:flex; flex-direction:column; }
    .workflow { flex:1; min-height:0; display:flex; flex-direction:column; justify-content:space-between; gap:3px; }
    .step {
      display:grid;
    ...
    .stepNumber {
      ...
      background:#e8eef6; color:#71849a;
    }
    .step.done { background:#f2fbf7; border-color:#ccebdd; }
    .step.done .stepNumber { background:#2ca66f; color:#fff; }
    .step.active { border-color:color-mix(in srgb, var(--blue) 48%, white); background:color-mix(in srgb, var(--blue) 10%, white); }
    .step.active .stepNumber { background:linear-gradient(180deg,var(--blue),var(--blue2)); color:#fff; }
    .step.skipped { opacity:.46; }
    .stepName { font-size:12.8px; font-weight:950; }
    .stepDesc { color:var(--muted); font-size:9.8px; margin-top:2px; line-height:1.22; }
    .stepTime { color:#61748b; font-size:9.4px; font-weight:900; text-align:right; max-width:90px; }

    .timeCard { flex:0 0 auto; }
    .timerGrid { display:grid; grid-template-columns:repeat(2,1fr); gap:8px; }
    .timerCard { padding:9px 10px; border-radius:12px; background:rgba(255,255,255,0.72); border:1px solid #dbe6ef; }
    .timerCard span { display:block; color:var(--muted); font-size:8.8px; font-weight:950; letter-spacing:.6px; text-transform:uppercase; margin-bottom:3px; }
    .timerCard strong { font-size:20px; font-weight:950; }
    .timerNote { margin-top:7px; color:var(--muted); font-size:9.8px; line-height:1.3; }

    .operatorCard { flex:0 0 auto; }
    .stageHero {
      padding:14px;
      border-radius:13px;
      background:linear-gradient(135deg, color-mix(in srgb,var(--blue) 13%,white), #ffffff 78%);
    ...
    .stageBadge {
      ...
      font-size:11px; font-weight:950; letter-spacing:.7px; text-transform:uppercase;
    }
    .stageText { margin-top:8px; font-size:22px; font-weight:950; }
    .stageDetails { margin-top:5px; color:var(--muted); font-size:11.5px; line-height:1.35; white-space:pre-line; }
    .prompt {
      margin-top:8px; padding:12px; min-height:78px; border-radius:12px;
      background:rgba(255,255,255,0.80); border:1px solid #d7e2ec;
      font-size:13.2px; line-height:1.38; white-space:pre-line;
    }
    .actions { display:flex; flex-wrap:wrap; gap:8px; margin-top:8px; }
    .noAction { flex:1; padding:10px; border-radius:10px; border:1px dashed #cbd7e5; color:var(--muted); text-align:center; font-size:10.5px; }
    button {
      min-width:138px; padding:10px 13px; border:1px solid rgba(0,0,0,.06); border-radius:11px;
      font-size:13px;
      color:#fff; font-weight:900; cursor:pointer;
      background:linear-gradient(180deg,#72869d,#586d84);
      box-shadow:0 4px 11px rgba(40,61,84,.14);
    }
    button:hover { filter:brightness(1.04); }
    button.primary { background:linear-gradient(180deg,var(--blue),var(--blue2)); }
    button.success { background:linear-gradient(180deg,#36b77d,#238e60); }
    button.warning { background:linear-gradient(180deg,#eeab4d,#cf7e22); }
    button.abort { width:100%; margin-top:7px; background:linear-gradient(180deg,#e86b74,#c94752); }

    .cosconCard { flex:1 1 auto; min-height:0; }
    .metrics { display:grid; grid-template-columns:repeat(2,1fr); gap:6px; }
    .metric { padding:9px 10px; border-radius:12px; background:rgba(255,255,255,0.74); border:1px solid #dbe5ee; min-width:0; }
    .metric.wide { grid-column:1 / -1; }
    .metricLabel { color:var(--muted); font-size:8.8px; font-weight:950; letter-spacing:.7px; text-transform:uppercase; margin-bottom:3px; }
    .metricValue { font-size:18px; font-weight:950; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    .metricSub { margin-top:3px; color:var(--muted); font-size:9.4px; line-height:1.26; }

    .ok { color:#238e60; }
    .bad { color:#c94752; }
    .warningText { color:#b36b18; }

    .pidBox { padding:10px; border-radius:12px; border:1px solid #cfdff8; background:rgba(255,255,255,0.76); font-size:10.6px; }
    .pidRow { display:grid; grid-template-columns:1fr auto; gap:6px; margin-top:6px; }
    input { min-width:0; padding:8px; border-radius:9px; border:1px solid #ccd7e4; background:#fff; color:var(--text); font-weight:850; outline:none; }
    .pidStatus { margin-top:6px; color:#46688f; font-size:9.2px; line-height:1.28; }

    .errorBox { min-height:50px; padding:10px; border-radius:12px; border:1px solid #ccebdd; background:rgba(255,255,255,0.76); color:#247d55; font-size:10.4px; line-height:1.34; white-space:pre-line; }
    .errorBox.active { border-color:#f0c5ca; background:#fff5f6; color:#ad3e48; }
    .footer { color:var(--muted); font-size:10.3px; line-height:1.42; padding:0 1px; }
    .interlockHelp { margin-top:8px; padding:9px 10px; border-radius:11px; background:rgba(255,255,255,0.72); border:1px solid #ddd5f6; color:#655486; font-size:9.4px; line-height:1.32; }


    /* Final readability pass for the supervised Phase 02 operator view. */
    body { font-size:14px; }
    .topStats { grid-template-columns:repeat(4,minmax(120px,1fr)); gap:8px; }
    .topStat span { font-size:10px; }
    .topStat strong { font-size:18px; }
    .topStat small { font-size:9.2px; }
    .progressMeta { font-size:10px; }

    .sectionTitle {
      font-size:15.5px;
      -webkit-text-stroke:0.35px #000;
      text-shadow:-0.4px 0 #000, 0 0.4px #000, 0.4px 0 #000, 0 -0.4px #000;
    }
    .accent-violet .sectionTitle { color:#5939a4; }
    .accent-blue .sectionTitle { color:#245fbd; }
    .accent-cyan .sectionTitle { color:#146f82; }
    .accent-amber .sectionTitle { color:#a65a09; }
    .accent-pink .sectionTitle { color:#982c56; }
    .accent-green .sectionTitle { color:#126b41; }
    .accent-slate .sectionTitle { color:#4f6278; }
    .pill { font-size:9.8px; padding:4px 8px; background:rgba(255,255,255,.78); }

    .stageBadge { font-size:10px; padding:5px 9px; }
    .stageText { font-size:23px; }
    .stageDetails { font-size:12.2px; }
    .prompt {
      padding:11px 12px;
      min-height:72px;
    ...
    .noAction { font-size:11px; }
    button { font-size:13.5px; }

    .metrics { gap:8px; }
    .metricLabel { font-size:9.4px; }
    .metricValue { font-size:19px; }
    .metricSub { font-size:10px; }

    .timerCard span { font-size:9.4px; }
    .timerCard strong { font-size:21px; }
    .timerNote { font-size:10.2px; }
    .errorBox { font-size:11px; }
    .footer { font-size:10.8px; }
    .interlockHelp { font-size:10px; }
    .pidBox { font-size:11px; }
    .pidStatus { font-size:10px; }
    input { font-size:12px; }

    .workflow { gap:5px; }
    .step { min-height:54px; }
    .stepName { font-size:13.5px; }
    .stepDesc { font-size:10.2px; }
    .stepTime { font-size:9.8px; max-width:96px; }
    ...
    @media (max-width:1180px), (max-height:760px) {
      ...
      .rightColumn { grid-column:1 / -1; display:flex; }
      .workflowCard { min-height:580px; }
    }
...
<body>
  ...
  <div class="shell">
    <header class="card header accent-blue">
      <div>
        <div class="eyebrow">ICN2 · NPG chamber · Phase 02</div>
        <div class="title">Automated Sputtering-Annealing</div>
        <div class="subtitle">Direct COSCON UDP control. The COSCON webpage is not used. Only the argon leak valve remains manual.</div>
      </div>
      <div>
        <div class="topStats">
          <div class="topStat"><span>Run</span><strong id="runName">--</strong></div>
          <div class="topStat"><span>Cycle</span><strong id="cycleText">0 / 0</strong></div>
          <div class="topStat"><span>Current stage</span><strong id="headerStage">INIT</strong></div>
          <div class="topStat"><span>Safety interlock</span><strong id="headerInterlock">--</strong><small>Hardware permission chain</small></div>
        </div>
        <div class="progressWrap">
          <div class="progressLine"><div id="progressFill" class="progressFill"></div></div>
          <div class="progressMeta"><span id="progressText">Workflow progress: 0%</span><span>First integrated runs must remain supervised</span></div>
        </div>
      </div>
    </header>

    <main class="main">
      <section class="column leftColumn">
        <div class="card section operatorCard accent-blue">
          <div class="sectionHeader">
            <div class="sectionTitle">Operator guidance</div>
            <div class="pill">Only required actions appear</div>
        ...
        <div class="card section cosconCard accent-amber">
          <div class="sectionHeader">
            <div class="sectionTitle">COSCON and chamber</div>
            <div id="cosconHealth" class="pill">Waiting</div>
          </div>
          <div class="metrics">
            <div class="metric wide">
              <div class="metricLabel">COSCON mode</div>
              <div id="cosconMode" class="metricValue">--</div>
              <div id="cosconDetails" class="metricSub">No status yet</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Safety interlock</div>
              <div id="interlockVal" class="metricValue">--</div>
              <div id="interlockSub" class="metricSub">Waiting for safety-chain status</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Chamber pressure</div>
              <div id="pressureVal" class="metricValue">--</div>
              <div id="pressureSub" class="metricSub">Target near 2×10⁻⁵ mbar</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Energy</div>
              <div id="energyVal" class="metricValue">-- V</div>
              <div id="energySub" class="metricSub">Target: 2250 V</div>
            </div>
            <div class="metric">
              <div class="metricLabel">Emission</div>
              <div id="emissionVal" class="metricValue">-- mA</div>
              <div id="emissionSub" class="metricSub">Target: 10 mA</div>
            </div>
            <div class="metric wide">
              <div class="metricLabel">Filament current</div>
              <div id="filamentVal" class="metricValue">-- A</div>
              <div class="metricSub">COSCON monitor readback</div>
            </div>
          </div>
        </div>
      </section>

      <section class="column centerColumn">
        <div class="card section timeCard accent-cyan">
          <div class="sectionHeader">
            <div class="sectionTitle">Time information</div>
            <div class="pill">Live</div>
          </div>
          <div class="timerGrid">
            <div class="timerCard"><span id="currentTimerLabel">Current step</span><strong id="currentTimer">--:--</strong></div>
            <div class="timerCard"><span>Stage elapsed</span><strong id="stageElapsed">00:00</strong></div>
            <div class="timerCard"><span>Run elapsed</span><strong id="runElapsed">00:00</strong></div>
            <div class="timerCard"><span>Known timed work left</span><strong id="knownRemaining">--:--</strong></div>
          </div>
          <div class="timerNote">Degas and oven heating are device-dependent. During Degas, the countdown is the safety timeout—not an expected completion time.</div>
        </div>

        <div class="card section accent-pink">
          <div class="sectionHeader"><div class="sectionTitle">Oven and auxiliary readback</div><div class="pill">Live</div></div>
          <div class="metrics">
        ...
        <div id="pidControlCard" class="card section accent-blue" style="display:none">
          <div class="sectionHeader"><div class="sectionTitle">Live PID SV control</div><div class="pill">S1 write + verify</div></div>
          <div class="pidBox">
            <div>Use only when the operator intentionally needs to change the oven setpoint.</div>
        ...
        <div class="card section accent-green">
          <div class="sectionHeader"><div class="sectionTitle">System status</div><div id="updatedAt" class="pill">No telemetry</div></div>
          <div id="errorBox" class="errorBox">No active errors.</div>
        </div>

        <div class="card section accent-slate">
          <div class="sectionHeader"><div class="sectionTitle">Operator reminders</div></div>
          <div class="footer">
            Keep the COSCON webpage and SpecsLab/Prodigy closed while Phase 02 is active.<br><br>
            Degas, target validation, Operate, verification, sputtering timing and Standby are automated. The leak valve remains manual.<br><br>
            Supervise the first complete three-cycle runs and keep the local COSCON controls accessible.
          </div>
          <div class="interlockHelp"><strong>INTERLOCK:</strong> external hardware safety permission. <strong>OK</strong> permits operation; <strong>Tripped</strong> means the safety chain is open and activation must be blocked or stopped.</div>
        </div>
      </section>

      <section class="column rightColumn">
        <div class="card section workflowCard accent-violet">
          <div class="sectionHeader">
            <div class="sectionTitle">Current-cycle workflow</div>
            <div class="pill">Live sequence</div>
          </div>
...
<script>
  ...
  function setHealth(snap) {
    ...
    const healthy = interlock.toLowerCase() === 'ok' && mode.toLowerCase() !== 'error';
    const health = el('cosconHealth');
    health.textContent = healthy ? 'COSCON healthy' : 'Check COSCON';
    health.className = `pill ${healthy ? 'ok' : 'bad'}`;
    el('headerInterlock').textContent = interlock;
    el('headerInterlock').className = healthy ? 'ok' : 'bad';
    el('interlockVal').textContent = interlock;
    el('interlockVal').className = `metricValue ${healthy ? 'ok' : 'bad'}`;
    el('interlockSub').textContent = interlock.toLowerCase() === 'ok'
      ? 'External safety chain permits operation'
      : 'Safety chain is tripped: activation must remain blocked';
  }

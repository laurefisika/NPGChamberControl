...
keeps the automation logic explicit and easier to maintain.
...
def update_live_plot(snapshot, force_autoscale=False):
    ...
    _update_axis(
        ax_temperature_oven, line_temperature_oven, oven_temp_times, oven_temp_values, 'oven_temp',
        f'{oven_last:.1f} ºC' if oven_last is not None else '--'
    )
    _update_axis(
...
def main():
    ...
    fax_temperature_oven.plot(data['Oven PID temperature']['temperature_times'], data['Oven PID temperature']['temperature_data'], '-o', color='magenta', markersize=4)
    fax_temperature_oven.set_title('Oven PID Temperature'); fax_temperature_oven.set_xlabel(''); fax_temperature_oven.set_ylabel('Temperature (ºC)'); fax_temperature_oven.tick_params(axis='x', rotation=30)

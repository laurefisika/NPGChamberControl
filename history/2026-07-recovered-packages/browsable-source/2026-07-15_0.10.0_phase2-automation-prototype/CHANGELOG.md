...
## 0.10.0 - Automated COSCON control in Phase 02

- Replaced the embedded COSCON web page in Phase 02 with a native status panel and direct UDP control on port 2005.
- Added a reusable, thread-safe `npg_chamber/devices/coscon_udp.py` client with a strict command whitelist. It supports only the read, Degas, Validate, Operate, Standby and Off commands required by the workflow; Reset, network changes and preset editing are deliberately unavailable.
- Automated the complete initial Degas from Off, including fresh XGS600 pressure preflight, `Degassing` state verification, continuous pressure/interlock monitoring and natural completion in Standby.
- Automated each sputtering start with pressure stabilization, target validation, one non-retried `SwitchToOperate` request, state verification and five consecutive valid measured energy/emission/pressure samples before the timer starts.
- Added continuous safety verification throughout every sputtering countdown: Mode, interlock, measured energy, emission and chamber pressure must remain valid. A major energy collapse, repeated deviation, pressure emergency, device Error, communication fault or operator abort enters the COSCON safe-stop path.
- Automated `Operating -> Standby` before valve closure and annealing. The manual argon leak valve remains the only non-automated Phase 02 process device in this release.
- Updated the Phase 02 GUI, telemetry CSV, launcher parameter editor, SOP, explanation PDF and developer tests.
- Preserved `original_scripts_backup/` unchanged as the pre-clean v0.9.12 recovery/reference copy.

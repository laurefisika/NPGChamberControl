...
PHASE_PARAMETER_SPECS: dict[str, tuple[ParameterSpec, ...]] = {
    ...
    "sputter": (
        ...
        ParameterSpec("coscon_degas_timeout_minutes", "Automatic Degas timeout", 20.0, unit="min", group="COSCON automation", description="Maximum time allowed for the complete initial Degas to finish naturally in Standby.", minimum=1, maximum=120),
        ParameterSpec("sputter_minutes", "Sputtering duration", 20.0, unit="min", group="Workflow", description="Verified COSCON Operating duration for each sputtering step.", minimum=0, maximum=1440),
        ParameterSpec("coscon_emission_target_a", "COSCON emission target", 0.010, unit="A", group="COSCON recipe", description="Emission target sent to ValidateOperateTarget and SwitchToOperate.", minimum=1e-6, maximum=0.1),
        ParameterSpec("coscon_energy_target_v", "COSCON energy target", 2250.0, unit="V", group="COSCON recipe", description="Ion energy target sent to ValidateOperateTarget and SwitchToOperate.", minimum=1, maximum=5000),
        ParameterSpec("coscon_stable_samples", "Stable COSCON samples", 5, kind="int", group="COSCON automation", description="Consecutive valid energy, emission and pressure readings required before the sputtering timer starts.", minimum=1, maximum=100),
        ParameterSpec("target_ar_pressure_mbar", "Target Ar pressure", 2.0e-5, unit="mbar", group="Pressure automation", description="Center of the pressure-stability band before automatic high-voltage activation.", minimum=1e-12, maximum=1),
        ParameterSpec("pressure_min_mbar", "Minimum sputtering pressure", 1.0e-5, unit="mbar", group="Pressure automation", description="Lower pressure limit accepted for automatic activation and continued sputtering.", minimum=1e-12, maximum=1),
        ParameterSpec("pressure_warning_mbar", "Maximum sputtering pressure", 5.0e-5, unit="mbar", group="Pressure automation", description="Upper normal-operation pressure limit; repeated violations stop sputtering.", minimum=1e-12, maximum=1),
        ParameterSpec("pressure_stable_seconds", "Pressure stability time", 60.0, unit="s", group="Pressure automation", description="Continuous time in the target pressure band required before automatic COSCON activation.", minimum=1, maximum=1800),
        ParameterSpec("pressure_stable_relative_band", "Pressure stability band", 0.08, unit="fraction", group="Pressure automation", description="Allowed fractional deviation around the target pressure during pre-activation stabilization.", minimum=0.001, maximum=0.5),
...
def validate_phase_values(phase: str, values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if phase in {"heat", "dpdbba"}:
        ...
        if not (
            normalized["pressure_min_mbar"]
            < normalized["target_ar_pressure_mbar"]
            < normalized["pressure_warning_mbar"]
        ):
            raise ValueError(
                "The target pressure must satisfy minimum < target pressure < maximum."
            )
        low_band = normalized["target_ar_pressure_mbar"] * (
            1.0 - normalized["pressure_stable_relative_band"]
        )
        high_band = normalized["target_ar_pressure_mbar"] * (
            1.0 + normalized["pressure_stable_relative_band"]
        )
        if low_band < normalized["pressure_min_mbar"] or high_band > normalized["pressure_warning_mbar"]:
            raise ValueError(
                "The complete pressure-stability band must fit inside the normal sputtering limits."
            )

...
class NPGLauncherApp:
    ...
    def open_automation_parameters(self) -> None:
        ...
        dialog.geometry("1280x900")
        dialog.minsize(940, 660)
        dialog.resizable(True, True)
        dialog.configure(bg="#f7f9fc")
        dialog.grab_set()

        # Keep the normal Windows title bar, including Maximize/Restore.  A
        # transient tool window can lose those controls on Windows, which made
        # this long editor unnecessarily cramped.  Open maximized when the
        # platform supports it and fall back to the large geometry above.
        def maximize_dialog() -> None:
            try:
                dialog.state("zoomed")
                return
            except tk.TclError:
        ...
        dialog.after_idle(maximize_dialog)
